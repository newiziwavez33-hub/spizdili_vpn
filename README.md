...
<h1 align="center">🦝 SPIZDILI_VPN (v1.2.1)</h1>
...
2. Скачайте **`SPIZDILI_VPN.exe`** (или `SPIZDILI_VPN_v1.2.1_Windows_x64.zip`).
...
Скачайте **`spizdili-vpn_1.2.1_amd64.deb`** из [последнего релиза](https://github.com/newiziwavez33-hub/spizdili_vpn/releases/latest) и установите:
```bash
sudo dpkg -i spizdili-vpn_1.2.1_amd64.deb
```
...
```bash
dpkg-deb --root-owner-group -b package_dir spizdili-vpn_1.2.1_amd64.deb
```

### Сборка standalone `.exe` для Windows (PyInstaller):
```bash
python windows/build_exe.py
# Создает dist/SPIZDILI_VPN.exe и dist/SPIZDILI_VPN_v1.2.1_Windows_x64.zip
```
